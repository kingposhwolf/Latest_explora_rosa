...
public interface CommentRepository extends CrudRepository<Comment, Long> {
    ...
    @Query("SELECT DISTINCT l.commenter.id as profileId, " +
        "l.likes as likes, " +
        "l.commenter.verificationStatus as verificationStatus, " +
        "l.commenter.user.username as username, " +
        "l.commenter.user.name as name, " +
        "l.message as message, " +
        "l.id as id, " +
        "l.timestamp as timestamp, " +
        "(SELECT COUNT(r.id) FROM Comment r WHERE r.parentComment.id = l.id) as replies " +
        "FROM Comment l WHERE l.userPost.id = :postId AND l.parentComment IS NULL " +
        "ORDER BY l.id ASC") // ASC for ascending order
List<Map<String, Object>> findCommentsForPost(@Param("postId") Long postId);



@Query("SELECT DISTINCT l.commenter.id as profileId, l.likes as likes, l.commenter.verificationStatus as verificationStatus, l.commenter.user.username as username, l.commenter.user.name as name, l.message as message, l.id as id, l.timestamp as timestamp,l.parentComment.id as parentComment  FROM Comment l WHERE  l.parentComment.id = :parentId AND l.userPost.id = :postId")
