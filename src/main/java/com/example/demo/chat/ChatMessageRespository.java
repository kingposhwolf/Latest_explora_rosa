...
public interface ChatMessageRespository extends CrudRepository<ChatMessage, Long>{
    ...
    @Query("SELECT cm.id as id,cm.sender.id as senderId, cm.content as content, cm.recipient.id as recipientId, cm.status as status FROM ChatMessage cm WHERE cm.chatRoom = :chatRoom")
    ...
    @Query("SELECT cm.id as id, cm.sender.id as senderId, cm.content as content, cm.recipient.id as recipientId, cm.status as status " +
    "FROM ChatMessage cm " +
    "WHERE cm.chatRoom = :chatRoom " +
    "AND cm.timestamp = (SELECT MAX(cm2.timestamp) FROM ChatMessage cm2 WHERE cm2.chatRoom = :chatRoom)")
    List<Map<String, Object>> lastMessage(ChatRoom chatRoom);
